...
for n, size in enumerate(component_sizes):
    start_idx = idx
    end_idx = idx + size
    taus[start_idx:end_idx] = scaling_factors[n] 
    idx = end_idx 

phi = bym2_multicomp_gamma_ppc_fit.stan_variable('phi')
phi_var_by_region = np.var((np.sqrt(1/taus) * phi), axis=0)
print('mean variance of phi scaled', np.mean(phi_var_by_region))

theta = bym2_multicomp_gamma_ppc_fit.stan_variable('theta')
...
```{python}
phi = bym2_1comp_gamma_ppc_fit.stan_variable('phi')
phi_var_by_region = np.var((np.sqrt(1/tau) * phi), axis=0)
print('mean variance of phi scaled', np.mean(phi_var_by_region))

theta = bym2_1comp_gamma_ppc_fit.stan_variable('theta')
print('mean variance of theta', np.mean(np.var(theta, axis=0)))
```
...
phi_multicomp_mean = np.mean(phi_multicomp_padded, axis=0)
...
plot_heatmap(nyc_gdf, phi_diff,
             ...
             "BYM2_multicomp_fit - BYM2_1comp_fit", "difference", (8, 7))
```
