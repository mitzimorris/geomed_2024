...
\vspace*{4pt}
* Interfaces and Packages for Validation and Visualization
  + R: [CmdStanR](https://mc-stan.org/cmdstanr/), [Posterior](https://mc-stan.org/posterior/), [Bayesplot](https://mc-stan.org/bayesplot/), [LOO](https://mc-stan.org/loo/)
...
* *Solution*: Many interfaces (R, Python, Julia, MATLAB), domain-specific examples, case studies, and field guides
...
* Stan Syntax
  + Syntax influenced by BUGS (and JAVA - semicolons at end of statements,\
curly braces around blocks)
  ...
  + Explicit variable types (like C, C++, JAVA, not like Python, R, BUGS)

* Stan Math Library
  + Rich set of probability distributions and mathematical functions available as part of the language.
...
births_fit = births_model$sample()
as.data.frame(births_fit$summary())
...
Faster ways to a less exact answer
\vspace{0.1in}
...
* Higher Level Interface for R: BRMS - Bayesian Regression Models in Stan
  ...
  + BRMS translates formula to Stan program, compiles, fits model to data
...
Install Stan
Run CmdStanR or CmdStanPy

Run Laplace Stan

Modify Laplace Stan, use supplied inputs


# Getting Started:  Notebook 2

Poisson regression:  disease rate per area, complete data:  all events recorded, population known

# Getting Started:  Notebook 3

Covid model - binomial regression:  disease rate per area, convenience sample - use MRP to post-stratify

Workflow:  generating datasets,  prior predictive check, posterior predictive checks

Add timeseries -

Retrospective studies:  estimating test positivity
...
add ICAR component to covid model

# Spatial Models - BYM2

workflow:  BYM2 vs simple ICAR
