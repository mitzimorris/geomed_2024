...
\vspace*{8pt}
* See handout:  `install_stan.html`


# Notebook Zero: Case Study Areal Data 
...
* Handouts
  * `spatial_data.html`
  * R version `spatial_data.Rmd`
  * Python version: `spatial_data.ipynb`


# Poisson Regression for Areal Data

Stan's Poisson-log Distribution

- **`poisson_log`** distribution uses the log rate as a parameter

- don't need to exponentiate, better numerical stability

- **`poisson_log_rng`** pseudo-random number generator function
...
argument to `poisson_log_rng` must be less than $30 \log 2$, $\approx 28$
  ...
  + used in the data generating model
...
# Notebook One:  Regression Models in Stan

This notebook follows the Bayesian Workflow recommendations by starting with a too-simple model
and then expanding it in multiple ways.
