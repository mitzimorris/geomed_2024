...
splot.plot_spatial_weights(w_nyc_nbs, nyc_geodata)
```
...
plot(nyc_geodata['geometry'], col='lightblue', alpha=0.1)
plot(nyc_nbs, coords=nyc_coords, add=TRUE, pch=21, cex=0.6)
```
...
plot(brooklyn_geodata['geometry'], col='skyblue', alpha=0.1)
plot(brooklyn_nbs, coords=brooklyn_coords, add=TRUE, pch=21, cex=0.6)
```
