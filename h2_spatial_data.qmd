...
### Manipulating the neighborhood graph - removing spurious connections

The neighborhood graph of New York City given the set of US Census shapefiles for the 2010 Census Tracts
has a few anomolies, showing multiple adjacencies between parts of Manhattan, Brooklyn, and Queens.
This may be due to bridges, tunnels, and subways, or it may simply be due to the way that the map was drawn.
For our analysis, a study of pedestrian traffic accidents, Manhattan is an island.
To edit the neighborhood graph, it is necessary to edit the neighborhood weights objects.
...
# re-create the W object with the updated neighbors and weights
...
** R **
```{r}
# remove specific neighbors from the nb object
remove_neighbors <- function(nb_object, target_indices, remove_indices) {
  for (i in target_indices) {
    # Keep neighbors that are NOT in the remove_indices
    nb_object[[i]] <- setdiff(nb_object[[i]], remove_indices)
  }
  return(nb_object)
}
...
nyc_nbs <- remove_neighbors(nyc_nbs, manhattan_indices, c(brooklyn_indices, queens_indices))
nyc_nbs <- remove_neighbors(nyc_nbs, brooklyn_indices, manhattan_indices)
nyc_nbs <- remove_neighbors(nyc_nbs, queens_indices, manhattan_indices)
```

Redraw maps to check updated neighborhood graphs.
...
### Manipulating the neighborhood graph - adding connections
